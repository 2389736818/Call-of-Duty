...
public void Client_ChangeClassTo(int client, ClassType type)
{
  if(IsPlayerAlive(client))
  {
    if(i_gCurrentClass[client] == Class_None)
    {
      ...
      Class_SetUp(client);
      PrintToChat(client, "%s %t %s", "Now you", CHAT_TAG, ClassNames[view_as<int>(type)]);
    ...
    else
    {
      ...
      PrintToChat(client, "%s %t %s", "Your class is changed at the start of the next round to", CHAT_TAG, ClassNames[view_as<int>(type)]);
    }
  }
  else
  {
    i_gCurrentClass[client] = type;
    PrintToChat(client, "%s %t %s!", "You changed the class to", CHAT_TAG, ClassNames[view_as<int>(type)]);
  }
}
